...
Contact:     @JankeSkanke
Created:     2021-01-02
Updated:     2023-13-03 by @modaly_it
...
2.0.0 - (2021-08-29) Moved secrets out of code - now running via Azure Function
...
3.7.1 - (2023-17-07) Added AV inventory collection
4.0.0 - () Support for backend now using new Log Ingestion API
#>
...
if ($CollectDeviceInventory) {
    ...
    $Inventory | Add-Member -MemberType NoteProperty -Name "TimeGenerated" -Value "$Date" -Force
	...
	$Inventory | Add-Member -MemberType NoteProperty -Name "TPMActivated" -Value "$ComputerTPMActivated" -Force
...
if ($CollectAppInventory) {
	...
	foreach ($App in $CleanAppList) {
		...
		$tempapp | Add-Member -MemberType NoteProperty -Name "TimeGenerated" -Value "$Date" -Force
...
# If additional logs is collected, remember to add to log payload 
$Outputdate =  $Date.ToString("yyyy-MM-dd HH:mm")
$OutputMessage = "InventoryDate:$Outputdate "

$LogPayLoad = New-Object -TypeName PSObject 
$DeviceInventory = ConvertTo-Json -InputObject $DeviceInventory -Depth 9 
$LogPayLoad | Add-Member -NotePropertyMembers @{$DeviceLogName = $DeviceInventory }
$AppInventory = ConvertTo-Json -InputObject $AppInventory -Depth 9
$LogPayLoad | Add-Member -NotePropertyMembers @{$AppLogName = $AppInventory }

$BodyTable = [ordered]@{
	DeviceName = $ComputerName
	EntraTenantID = $AzureADTenantID
	EntraDeviceID = $AzureADDeviceID
	Signature = $Signature
...
try {
	$ResponseInventory = Invoke-RestMethod $AzureFunctionURL -Method 'POST' -Headers $headers -Body ($BodyTable | ConvertTo-Json -depth 9) -ContentType "application/json"
	foreach ($response in $ResponseInventory) {
		if ($response.response -match "204") {
